...
function replaceVersion(array &$requireSection, array &$requireDevSection, array $packages, $version) {
	foreach ($packages as $package) {
		if (array_key_exists($package, $requireSection)) {
			$requireSection[$package] = $version;
		}
		if (array_key_exists($package, $requireDevSection)) {
			$requireDevSection[$package] = $version;
		}
	}
}
...
if ($netteVersion) {
	...
	replaceVersion($composer['require'], $composer['require-dev'], $nettePackages, $netteVersion);
}
